...
export default function ResumenAnual() {
  ...
  const mensual = MESES.map((mes, idx) => {
    ...
    const gas = movs.filter(m => m.tipo === 'Gasto').reduce((a,b) => a+Number(b.monto), 0)
    return { mes: mes.slice(0,3), Ingresos: ing, Gastos: gas, Balance: ing-gas, Ahorro: ing > 0 ? ((ing-gas)/ing*100).toFixed(1) : 0 }
  })
    ...
    <div>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <div className="page-subtitle">Vista completa del año</div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button className="month-btn" style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 8, padding: '6px 12px', color: 'var(--text)', cursor: 'pointer' }} onClick={() => setAnio(a=>a-1)}>‹</button>
          <span style={{ fontWeight: 700, fontSize: 18 }}>{anio}</span>
          <button className="month-btn" style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 8, padding: '6px 12px', color: 'var(--text)', cursor: 'pointer' }} onClick={() => setAnio(a=>a+1)}>›</button>
        </div>
      </div>
      ...
      <div className="stat-grid" style={{ marginBottom: 24 }}>
        <div className="stat-card green"><div className="stat-label">Total Ingresos {anio}</div><div className="stat-value green">{fmt(totalIng)}</div></div>
        <div className="stat-card red"><div className="stat-label">Total Gastos {anio}</div><div className="stat-value red">{fmt(totalGas)}</div></div>
        <div className="stat-card blue"><div className="stat-label">Balance anual</div><div className={`stat-value ${totalIng-totalGas >= 0 ? 'green' : 'red'}`}>{fmt(totalIng-totalGas)}</div></div>
        <div className="stat-card yellow"><div className="stat-label">% Ahorro anual</div><div className="stat-value yellow">{totalIng > 0 ? ((totalIng-totalGas)/totalIng*100).toFixed(1) : 0}%</div></div>
      </div>
      ...
      {tab === 'mensual' && <>
        ...
        <div className="table-wrap">
          <table>
            <thead><tr>
              <th>Mes</th><th>Ingresos</th><th>Gastos</th><th>Balance</th><th>% Ahorro</th>
            </tr></thead>
            <tbody>
              {mensual.map((m, i) => (
                <tr key={i}>
                  ...
                  <td><span className="monto-neg">{fmt(m.Gastos)}</span></td>
              ...
              <tr style={{ borderTop: '2px solid var(--border)' }}>
                ...
                <td><span className="monto-pos" style={{ fontSize: 15 }}>{fmt(totalIng)}</span></td>
                <td><span className="monto-neg" style={{ fontSize: 15 }}>{fmt(totalGas)}</span></td>
                <td><span className={totalIng-totalGas >= 0 ? 'monto-pos' : 'monto-neg'} style={{ fontSize: 15 }}>{fmt(totalIng-totalGas)}</span></td>
      ...
      {tab === 'categorias' && <>
            ...
            <tbody>
              {catsIngreso.map(([cat, val]) => (
                <tr key={cat}>
                  ...
                  {val.meses.map((m,i) => <td key={i}>{m > 0 ? <span className="monto-pos" style={{ fontSize: 12 }}>{fmt(m)}</span> : <span style={{ color: 'var(--border)' }}>-</span>}</td>)}
        ...
        <div className="section-title" style={{ color: 'var(--red)', marginBottom: 12 }}>📤 Gastos por categoría</div>
        <div className="table-wrap">
          <table>
            <thead><tr>
              <th>Categoría</th>
            ...
            <tbody>
              {catsGasto.map(([cat, val]) => (
                <tr key={cat}>
                  <td style={{ fontWeight: 600 }}>{cat}</td>
                  {val.meses.map((m,i) => <td key={i}>{m > 0 ? <span className="monto-neg" style={{ fontSize: 12 }}>{fmt(m)}</span> : <span style={{ color: 'var(--border)' }}>-</span>}</td>)}
